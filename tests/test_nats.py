...
import fastagency_studio.io.ionats
from fastagency_studio.app import add_model
from fastagency_studio.io.ionats import (  # type: ignore [attr-defined]
    InputResponseModel,
    ServerResponseModel,
    broker,
    stream,
...
class TestAutogen:
    ...
    async def test_ionats_success(  # noqa: C901
        ...
        monkeypatch.setattr(fastagency_studio.io.ionats, "create_team", create_team)

        async with TestNatsBroker(broker) as br:
            await br.publish(
                fastagency_studio.io.ionats.InitiateModel(
                    msg="exit",
                    thread_id=thread_id,
                    team_id=team_id,
                    user_id=user_id,
